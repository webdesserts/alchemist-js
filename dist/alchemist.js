...
Alchemist.prototype.convert = function (color, dest) {
  ...
  if (typeof converter === 'function') {
    if (isArray(color.value)) {
      value = converter.apply(this, clone(color.value).concat(color));
    } else {
      value = converter(clone(color.value), color)
    }
...
var clone = function (original) {
  ...
  throw new Error('Alchemist does not know how to clone ' + original)
}
